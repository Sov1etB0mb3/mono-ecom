...
interface CartSearchRepositoryInternal {
    Stream<Cart> search(String query);

    Stream<Cart> search(Query query);
...
class CartSearchRepositoryInternalImpl implements CartSearchRepositoryInternal {
    ...
    @Override
    public Stream<Cart> search(String query) {
        NativeQuery nativeQuery = new NativeQuery(QueryStringQuery.of(qs -> qs.query(query))._toQuery());
        return search(nativeQuery);
    }

    @Override
    public Stream<Cart> search(Query query) {
        return elasticsearchTemplate.search(query, Cart.class).map(SearchHit::getContent).stream();
    }
